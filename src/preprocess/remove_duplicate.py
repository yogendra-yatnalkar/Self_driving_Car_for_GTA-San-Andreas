    ...
    def rm_duplicate_img(self):
        if os.path.exists(self.csv_file_path):
            ...
            if os.path.exists(self.img_ds_path):
                    ...
                    if img_name not in img_db:
                        ...
                        if hash in self.hash_db:
                            ...
                            print(
                                "\n",
                                img_name,
                                "--- REMOVED from dataset and csv file ---",
                                "\n",
                            )
                ...
                print(
                    '\n"No. of corrupted csv entries found and deleted : ',
                    self.count_corrupt,
                )
                print(
                    '\n"No. of duplicate images found and deleted : ',
                    self.count_duplicate,
                )
                print("\nNo of unaccounted files : ", len(img_db))

                ds_df.to_csv(
                    os.path.join(self.csv_file_path, "dataset.csv"), index=False
                )

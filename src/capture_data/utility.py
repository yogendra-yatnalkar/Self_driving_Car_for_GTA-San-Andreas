...
class Utility:
    ...
    def merge_temp_ds(self, ds_path):
        csv_save_path = os.path.dirname(
            ds_path
        )  # csv files are stored in parent folder of dataset folder
        df_temp = pd.read_csv(
            os.path.join(csv_save_path, "temp.csv")
        )  # data_frame of temp.csv
        df_ds = pd.read_csv(
            os.path.join(csv_save_path, "dataset.csv")
        )  # data_frame of dataset.csv
